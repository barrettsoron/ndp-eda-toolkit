# Canvasser Guide: Surveillance Pricing Petition

**Vancouver Centre NDP | For volunteer use**
*Data and sources reviewed and verified as of April 15, 2026. Check parliamentary status of Motion M-30 closer to use.*

---

## The 30-second opener (door or table)

> "Hi — I'm a volunteer with the Vancouver Centre NDP. We're collecting signatures on a petition to ban surveillance pricing — it's when stores use your personal data to charge you more than your neighbour for the exact same item. The NDP is the only party putting forward a concrete fix. Can I tell you more, or would you like to sign?"

Use "unfair and downright creepy" if it feels right. It usually does.

---

## What surveillance pricing is (if they ask)

When you shop online — or use your loyalty card in-store — the retailer collects data about you: where you live, when you shop, what you've bought before, how price-sensitive you seem. An AI algorithm processes all of it and figures out the highest price you personally will pay. Then it charges you that.

Two people buying the same box of cereal can pay different amounts, and neither of them knows it.

It's already spreading into groceries, rental housing, ride-sharing, concert tickets, utilities, and pharmaceuticals.

---

## Concrete examples (use the one that lands)

**Sick kid:** "Imagine you're searching for children's Tylenol at 2am because your kid has a fever. The algorithm sees you're desperate — late night, limited options, stressed. That gets priced in."

**Death in the family:** "When someone dies, the family has to book flights and hotels on short notice. Airlines have always charged more for that. Now the algorithm does it automatically, in real time, across every sector."

**New parents:** "Buy diapers from the same retailer twice and the algorithm knows you'll be back. It starts walking the price up, because it's learned you don't have a choice."

**Loyalty card:** "Every time you scan your points card, you're giving the store a detailed map of your habits and needs. The points are worth cents. The data is worth a lot more — to them."

---

## Common objections

**"Isn't this just regular pricing? Supply and demand?"**

> "Dynamic pricing — prices going up when demand is high — is different. Surveillance pricing uses your *personal* data to figure out what *you specifically* will pay. Same demand, same store, different price for different people based on their data profile. That's not a market — that's a targeting system."

**"If I don't like it, I can shop somewhere else."**

> "That's getting harder. When multiple grocery chains use the same third-party pricing algorithm, they can effectively align on prices without any formal agreement. The algorithm does the coordination. You can't opt out by switching stores if all the stores are using the same system."

**"I don't have anything to hide."**

> "It's not about hiding — it's about being charged fairly. Your neighbour might pay less for the same groceries, not because they're a better shopper, but because their data profile is different. That's not competition. That's extraction."

---

## The ask

1. **Sign the paper petition** — collect name, address, postal code, email, and phone. All fields help, but don't push if someone hesitates on phone.
2. **Direct to the e-petition:** ndp.ca/ban-surveillance-pricing — or show the QR code if you have one printed.
3. **Offer to leave a fact sheet** — good for people who want to think about it or share with family.

---

## What to do with signed paper petitions

- Keep pages flat and dry — don't fold.
- Return to your chapter lead or bring to the next VCNDP event.
- If you collect email addresses, those can be entered into the NDP database by chapter leads — do not photograph or keep personal data on your personal devices.

---

## Notes

- This is a **party** action — Vancouver Centre NDP, not the Avi Lewis campaign. Keep branding party-level.
- The petition asks the **federal government** to act — relevant jurisdiction.
- Manitoba NDP has already introduced a provincial bill — good "it works" precedent if someone asks.

---

*Vancouver Centre NDP | April 2026*
