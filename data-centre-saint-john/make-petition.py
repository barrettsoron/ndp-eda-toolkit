...
def draw_form(c, top_y):
    ...
    c.drawString(MARGIN + BASE, bar_y + 27,
        'Pause Beacon’s Lorneville Data Centre')

    c.setFont('Helvetica', 6)
    c.drawString(MARGIN + BASE, bar_y + 13,
        'Saint John–St. Croix NDP │ Peace and Friendship Treaty territory'
        ' │ Their Gas. Our Wetlands. Our Choice.')
    ...
    c.setFillColor(DARK)
    summary = (
        'Beacon AI Centers (Calgary) and VoltaGrid (Texas) are proposing a 390 MW data centre '
        'at Lorneville — 190 MW from an on-site gas plant, 200 MW from NB Power’s '
        'grid — with no disclosed anchor tenants, no meaningful consultation, and the '
        'destruction of wetlands and old-growth forest on Peace and Friendship Treaty territory. '
        'Pause the project until our community has a say.'
    )
    ...
    y -= SECT_GAP
    stmt = (
        '“I call on the federal government to pause the expansion of AI data centres in Canada'
        ' — starting with Beacon AI Centers’ proposed 390 MW campus at Lorneville,'
        ' Saint John — until meaningful community consultation, environmental assessment,'
        ' and democratic oversight have taken place. Our energy, our wetlands, our water, and'
        ' our data sovereignty must serve Canadians first.”'
    )
    rule_x     = MARGIN + BASE
    text_x     = rule_x + BASE
    ...
    sig_bottom = draw_field(c, MARGIN, y, 'Signature', col_a, line_h=SIG_LINE_H)
    draw_field(c, MARGIN + col_a + col_gap, y, 'Date', col_b, line_h=SIG_LINE_H)

    # -- Layout check --------------------------------------------------------
    footer_y   = (top_y - FORM_H) + 4 * BASE
    footer_top = footer_y + 9 + 6.5  # top of two-line footer block
    if sig_bottom < footer_top + 4:
        raise LayoutError(
            f'Footer overlap: signature label bottom ({sig_bottom:.1f}) is above '
            f'footer top ({footer_top:.1f}). Shorten the summary or statement text.'
        )
    ...
    c.drawString(MARGIN, footer_y + 9,
        'Return signed forms to your chapter lead or any Saint John–St. Croix NDP event.')
...
out = os.path.join(os.path.dirname(__file__), 'data-centre-petition.pdf')
